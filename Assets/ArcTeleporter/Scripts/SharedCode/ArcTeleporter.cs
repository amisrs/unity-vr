...
public class ArcTeleporter : MonoBehaviour {
	...
	void Update () {
		...
		bool currentTriggerState = OVRInput.Get (OVRInput.RawButton.LIndexTrigger);
		...
		if (lastTriggerState && !currentTriggerState) {
			...
			if (arcRaycaster!= null && arcRaycaster.MakingContact) {
				if (objectToMove != null) {
                    ...
                    FadeAndTeleport(0.3f, arcRaycaster.HitPoint + up * height);
    ...
    void FadeAndTeleport(float fadeTime, Vector3 position)
    {
        float elapsedTime = 0.0f;
        screenFade.FadeOut(0.6f);
