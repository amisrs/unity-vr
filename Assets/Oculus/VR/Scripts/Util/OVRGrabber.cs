...
public class OVRGrabber : MonoBehaviour
{
    ...
    [SerializeField]
    protected List<GameObject> highlightedObjects = new List<GameObject>();
    protected GameObject previousHighlightedObject;
    ...
    void HighlightClosestGrabbable() {
        OVRGrabbable closestGrabbable = GetClosestGrabbable();
        if(closestGrabbable == null)
        {
            return;
        }
        if(previousHighlightedObject == closestGrabbable)
        {
            Debug.Log("highlighted object is still the same.");
            //donothing
        } else
        {
            Debug.Log("Theres a new closestgrabbable");
            if (previousHighlightedObject != null)
            {
                deactivateOutlineShader(previousHighlightedObject);
            }
            
            activateOutlineShader(closestGrabbable.gameObject);
            previousHighlightedObject = closestGrabbable.gameObject;
        }
    ...
    void OnTriggerExit(Collider otherCollider)
    {
		OVRGrabbable grabbable = otherCollider.GetComponent<OVRGrabbable>() ?? otherCollider.GetComponentInParent<OVRGrabbable>();
        if (grabbable == null) return;
        /*if (highlightedObjects.Contains(grabbable.gameObject))
        {
            //Debug.Log(grabbable.gameObject.name + " left the trigger, it was highlighted. Deactivating highlight.");
            highlightedObjects.Remove(grabbable.gameObject);
            deactivateOutlineShader(grabbable.gameObject);
        }*/
    ...
    protected OVRGrabbable GetClosestGrabbable()
    {
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            deactivateOutlineShader(m_grabbedObj.gameObject);
